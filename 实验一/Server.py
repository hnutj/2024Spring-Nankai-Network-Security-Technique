...
class SubThread_chat(threading.Thread):
    def run(self):
        global thelist,aes_fernet,choice,pk_partner,ecc_pk_partner,associ_key,cipher
        s1 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s1.bind(('127.0.0.1', 111))
        s1.listen(10)
        sock, addr = s1.accept()
...
def send():
    ...
    global v,aes_fernet,pk_partner,choice,s2,cipher
    if choice.get()==0:
        #des
        data=v.get().encode()
...
if __name__ == '__main__':
    ...
    with open("./pyCryptoChat/info_attached/server_rsa_privkey.pem", "wb") as f:
        f.write(
    ...
    with open("./pyCryptoChat/info_attached/server_rsa_pubkey.pem", "wb") as f:
        f.write(
    ...
    with open("./pyCryptoChat/info_attached/server_ecc_pubkey.pem", "wb") as f:
        f.write(
    ...
    p1.start()
    p2.start()
    s2=connect_with_retry("127.0.0.1",999)
     #verification of certifi
    veri_cert()
    with open("./pyCryptoChat/info_attached/client_rsa_pubkey.pem", "rb") as key_file:
        pk_partner = serialization.load_pem_public_key(
            key_file.read(),
        )
    with open("./pyCryptoChat/info_attached/client_ecc_pubkey.pem", "rb") as key_file:
